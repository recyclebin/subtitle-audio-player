...
`androidStopForegroundOnPause: false` keeps the notification visible when paused. The notification's `displayTitle` shows the current subtitle text; `displaySubtitle` shows the filename. Only the play/pause button appears in compact view (`androidCompactActionIndices: [0]`); prev/next are in the expanded view.
